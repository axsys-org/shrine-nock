...
++  spied
  ...
  |=  ran=type
  ^-  (unit type)
  =/  nok=(unit nock)  (mole |.((~(fish ut ran) 1)))
  ?~  nok  ~
  =/  res=(unit *)  (mole |.(.*(q.vax u.nok)))
  ?:  =([~ &] res)
    `ran
  ~
...
++  start-pail-spec
 ^-  spec
 [%bccl (thaw /std/pro/nul) [%base %null] ~]
++  thaw
  |=  pax=pith
  ^-  spec
  =;  el=(list spec)
...
++  freeze
  ...
  ~|  bad-freeze/-.p.vax
  ?-  -.p.vax
      %hold   $(p.vax ~(repo ut p.vax))
      %core   !! 
      %atom   [%leaf p.p.vax ;;(@ q.vax)]
      %face   $(p.vax q.p.vax)
      %hint   $(p.vax q.p.vax)
      %fork   $(vax (speed vax))
      %cell   
...
++  ut
  ...
  ++  play
    ...
    ?-  gen
      ...
      [%fits *]  bool
      [%wthx *]  bool
      [%dbug *]  ~>(%slog.[1 (show %o p.gen)] $(gen q.gen))
      [%zpcm *]  $(gen p.gen)
      [%lost *]  %void
...
++  vast                                                ::  main parsing core
  ...
  ++  weed 
    |^
    %-  sear
    :_  ;~(pfix cen fas (more fas twig))
    |=  ss=(list spec)
    ^-  (unit spec)
    ?~  ss  ~
    `[%bccl i.ss (snoc t.ss %base %null)]
    ++  twig  ;~(pose burl stem)
    ++  face
      ;~  pose
        (stag ~ ;~(sfix sym tis))
    ...
    ++  burl
      %+  cook
        |=  [face=(unit term) aura=term]
        ^-  spec
        =/  sk=(unit skin)  ?~(face ~ `u.face)
        =/  norm=spec  [%base %atom aura]
        ?~  sk  norm
        [%bcts u.sk norm]
      %+  ifix  [sel ser]
      ;~  plug
    ...
    ++  stem
      ...
      %+  cook
          ...
          ^-  spec
          ?@  iota  [%leaf %tas iota]
          [%bccl [%leaf %tas -.iota] [%leaf -.iota +.iota] ~]
      |^  %-  stew
          ^.  stet  ^.  limo
          :~  :-  'a'^'z'  (slot sym)
              :-  '$'      (cold %$ buc)
